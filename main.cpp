...
InterruptIn freq_chan1(D5, PullUp);             // Frequence channel 1
InterruptIn freq_chan2(D6, PullUp);             // Frequence channel 2
AnalogIn pot0(A0),
         pot1(A1),
         pot2(A2),
         pot3(A3),
...
uint16_t pulse_counter1 = 0,
         pulse_counter2 = 0;                    // Frequence counter variables
uint16_t acc_addr = 0;                          /* LSM6DS3 address, if not connected
                                                   address is 0 and data is not stored */
...
//void sendPack();
uint32_t count_files_in_sd(const char *fsrc);   // Compute number of files in SD
void freq_channel1_ISR();                       // Frequence counter ISR, channel 1
void freq_channel2_ISR();                       // Frequence counter ISR, channel 2
void toggle_logging();                          // Start button ISR
